...
- [x] T002 確定 AG-UI contract、ports、environment variables
  ...
  - Env: `VITE_AGENT_URL`（完整 `/agui` URL）、`AI_GATEWAY_API_KEY`、`AI_GATEWAY_BASE_URL`、`AGENT_MODEL_ID=google/gemini-3.5-flash-lite`、`AGENT_OS_*`、`AGENT_CORS_ORIGINS`
...
- [x] T004 Backend — 初始化 Agno AgentOS + `AGUI`，提供 `POST /agui` 與 `GET /status`；模型經 **Vercel AI Gateway**（`google/gemini-3.5-flash-lite`）
...
- [x] T005 End-to-end check、README、PR review
  ...
  - Streaming chat requires `AI_GATEWAY_API_KEY` (manual)
